...
def generate_launch_description():
    """
    Ros2 launch function to test flying a cf_publisher node with the `fly` node parameter set to `True`.
    This essentially runs the `drl_pkg.crazyflie_publisher.CrazyfliePublisher`. 
    ...
    Args:
        params_file (:obj:`str`, optional): The path to the experiment configuration file.
        Defaults to 'config/experiment_config.yaml'.

    Returns:
        :obj:`foxglove_websocket` node
        :obj:`crazyflie_node`: Included `cf_publisher_launch` LaunchDescription with parameters as: Fly = True,
        URI = 'radio://0/80/2M/E7E7E7E7E7', config_file = params_file.
    ...
    static_tf_publisher = launch_ros.actions.Node(
        ...
        output = 'screen',
        arguments = [str(initial_translation[0]),
